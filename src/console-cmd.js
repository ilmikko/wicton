...
console.command={
        modes:{
                'default':function parseInput(input){
        ...
        parse:function(input){
                if (!(this.mode in this.modes)) this.mode='default';
                this.modes[this.mode](input);
        },
        chmode:function(mode){
                if (!(mode in this.modes)) return console.error('Cannot change mode to \''+mode+'\' - mode doesn\'t exist!');
                this.mode=mode;
                console.warn('Console mode changed to '+mode);
        },
        addmode:function(mode,f){
                if (mode in this.modes) return console.error('Cannot add mode \''+mode+'\' - mode is already defined!');
                this.modes[mode]=f;
        },
        run:new Proxy({
...
console.command.add('chmode',function(mode){
        console.command.chmode(mode);
},{help:'Change the console input mode\nUsage: chmode [mode]\nPlease note that the mode must be defined.\nYou can inspect the available modes by running `inspect console.command.modes`'});
